...
don't need the \ensuremath{\Varid{set}} operation---or from an arbitrary Abelian group
(\ie commutative monoid with inverses) if we do need \ensuremath{\Varid{set}}---but we
...
\\[\blanklineskip]%
\>[B]{}\Varid{put}\mathbin{::}\Conid{Index}\to \Conid{Integer}\to \Conid{SegTree}\to \Conid{SegTree}{}\<[E]%
\\
\>[B]{}\Varid{put}\;\Varid{i}\;\Varid{v}\;\Varid{t}\mathrel{=}\Varid{update}\;\Varid{i}\;(\Varid{v}\mathbin{-}\Varid{get}\;\Varid{i}\;\Varid{t})\;\Varid{t}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\ensuremath{\Conid{Integer}}---how would we know when to stop?  In fact, these practical
...
\ensuremath{\Varid{cycle}\;[\mskip1.5mu \Conid{I},\Conid{O}\mskip1.5mu]} is an infinite list which alternates between \ensuremath{\Conid{I}} and \ensuremath{\Conid{O}}
...
\ensuremath{\Conid{I}} at every prime index and \ensuremath{\Conid{O}} everywhere else.
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{expand}\mathbin{::}\Conid{Bits}\to \Conid{Bits}{}\<[E]%
\\
\>[B]{}\Varid{expand}\;(\Conid{Rep}\;\Varid{a})\mathrel{=}\Conid{Snoc}\;(\Conid{Rep}\;\Varid{a})\;\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{expand}\;\Varid{as}\mathrel{=}\Varid{as}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{pattern}\;(\mathrel{:\!.})\mathbin{::}\Conid{Bits}\to \Conid{Bit}\to \Conid{Bits}{}\<[E]%
\\
\>[B]{}\Varid{pattern}\;(\mathrel{:\!.})\;\Varid{bs}\;\Varid{b}\leftarrow (\Varid{expand}\to \Conid{Snoc}\;\Varid{bs}\;\Varid{b}){}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}{}\<[E]%
\\
...
exercise for the interested reader to prove that \ensuremath{\Varid{x}\oplus\Varid{neg}\;\Varid{x}\equiv \mathrm{0}}.
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
...
\item Next, if \ensuremath{\Varid{x}\mathrel{=}\Varid{xs}\mathrel{:\!.}\Conid{I}}, then \ensuremath{\Varid{lsb}\;(\Varid{xs}\mathrel{:\!.}\Conid{I})\mathrel{=}\Conid{Rep}\;\Conid{O}\mathrel{:\!.}\mathrm{1}} by
  ...
  \begin{sproof}
    ...
    \stmt{\ensuremath{\mathrm{1}\mathbin{:}\Varid{zeros}}}
  \end{sproof}
\end{itemize}
...
We could alternatively define \ensuremath{\Varid{while}\;\Varid{p}\;\Varid{f}\mathrel{=}\Varid{head}\mathbin{\circ}\Varid{dropWhile}\;\Varid{p}\mathbin{\circ}\Varid{iterate}\;\Varid{f}}, but the more direct definition will be more convenient
for reasoning.
...
finding the closest active parent at each step by adding the \ensuremath{\Varid{lsb}} of
the current index:
