...
\begin{proof}
  ...
  $\ensuremath{\Varid{x}\mathrel{=}\Varid{xs}\mathrel{:\!.}\Conid{O}} \leq 2^n$, we have $\ensuremath{\Varid{xs}} \leq 2^{n-1}$ and
  $\ensuremath{\Varid{shift}\;\Varid{n}\;\Varid{xs}} \leq 2^n + 2^{n-1}$ (recall that \ensuremath{\Varid{shift}\;\Varid{n}\mathrel{=}\Varid{while}\;\Varid{even}\;\Varid{shr}\mathbin{\circ}\Varid{set}\;\Varid{n}} sets the $n$th bit and then can only make the number
